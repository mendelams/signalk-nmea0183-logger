...
module.exports = function (app) {
  ...
  function getCachedStats(fp, opts) {
    ...
    // 2. Disk cache (survives restarts) — only for default opts (no fullTrack, no AIS)
    if (!isFullTrack && !isAIS) {
    ...
    // 3. Parse fresh
    const data = parseLogFile(fp, config, t, opts);
    storeMemCache(memKey, mtime, data);

    // Write disk cache (only for default opts — track is too large for disk)
    if (!isFullTrack && !isAIS) {
      try {
