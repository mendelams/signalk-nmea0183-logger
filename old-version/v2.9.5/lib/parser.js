...
function processLines(lines, config, t, opts) {
  ...
  // ── Sentence types safe to skip without affecting any stat ────────────────
  // GSV = satellite visibility, GSA = fix mode, GST = error ellipse (all unused)
  // VTG = track/speed (redundant with RMC), TXT = text messages
  const SKIP_TYPES = new Set(['GSV','GSA','GST','VTG','TXT','GBS']);
  ...
  for (const line of lines) {
    ...
    // ── Checksum validation (NMEA $ sentences only) ───────────────────────
    if (c0 === 36 && s.indexOf('*') > 0) {
    ...
    // ── Early skip for known-unused sentence types ────────────────────────
    if (SKIP_TYPES.has(st)) {
      sentenceTypeCounts[st] = (sentenceTypeCounts[st] || 0) + 1;
      continue;
    }
