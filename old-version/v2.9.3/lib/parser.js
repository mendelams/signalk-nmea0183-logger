...
async function parseLogFileAsync(filepath, config, t, opts) {
  ...
  if (filepath.endsWith('.gz')) {
    // Streaming gunzip + line collection. For .gz we still need full
    // decompression but it's done off the main path via promisified gunzip.
    ...
    // True streaming for plain .log files: constant memory regardless of size
