...
async function viewFile(fn){return guard(async()=>{
  ...
  <div class="export-panel" id="export-panel">
    <div class="export-btns">
      ...
      <span class="btn btn-sm" onclick="showReplay('${fn}')">▶ ${t('replay')}</span>
    </div>
    <div class="export-opts">
      <label><input type="checkbox" id="exp-ais" ${P.gpxAis?'checked':''} onchange="setSave('gpxAis',this.checked);updateGpxUrl()"> AIS</label>
      <label><input type="checkbox" id="exp-ev" ${P.gpxEvents!==false?'checked':''} onchange="setSave('gpxEvents',this.checked);updateGpxUrl()"> Events</label>
...
/** Toggle the NMEA replay panel for a log file. Loads sentences on first open. */
async function showReplay(fn){
  // Close export panel
  const ep=document.getElementById('export-panel');
  if(ep) ep.style.display='none';

  // Check if replay panel exists — toggle
  const existing=document.getElementById('replay-panel');
  if(existing){stopReplay();existing.remove();return;}

  // Insert replay panel after export panel
  const after=ep||document.querySelector('.topbar');
  if(!after) return;
  const panel=document.createElement('div');
  panel.id='replay-panel';
  panel.className='card';
  panel.innerHTML=`<div class="card-title">▶ ${t('replay')}</div>
    <div style="padding:10px 14px">
      <div class="btn-row">
        <span class="btn btn-sm btn-blue" id="rp-play" onclick="startReplay('${fn}')">▶ ${t('replayStart')}</span>
        <span class="btn btn-sm" id="rp-pause" onclick="toggleReplayPause()" style="display:none">⏸</span>
        <span class="btn btn-sm btn-red" id="rp-stop" onclick="stopReplay()" style="display:none">⏹ ${t('replayStop')}</span>
      </div>
      <div class="eng-form-row" style="margin-top:8px">
        <label class="eng-field-label">${t('replaySpeed')}</label>
        <select id="rp-speed" class="set-select" onchange="replaySpeed=parseInt(this.value)">
      ...
      <div class="meta-text" id="rp-status"></div>
      <div class="maint-bar" style="margin-top:6px"><div class="maint-bar-fill" id="rp-progress" style="width:0%;background:var(--blue)"></div></div>
    </div>`;
  after.parentNode.insertBefore(panel,after.nextSibling);
  replaySpeed=10;
}

async function startReplay(fn){
  if(replayTimer){stopReplay();}
  const status=document.getElementById('rp-status');
  if(status) status.textContent=t('loading');
  try{
    replayData=await api('/api/logs/'+fn+'/replay');
  }catch(e){toast('Error: '+e.message,'err');return;}
  replayIdx=0;
  replayPaused=false;
  // Show controls
  const play=document.getElementById('rp-play');
  const pause=document.getElementById('rp-pause');
  const stop=document.getElementById('rp-stop');
  if(play) play.style.display='none';
  if(pause) pause.style.display='';
  if(stop) stop.style.display='';
  if(status) status.textContent=`0 / ${replayData.totalSentences} ${t('replaySentences')}`;
  replayNext();
}

function replayNext(){
  if(!replayData||replayIdx>=replayData.sentences.length){
    const status=document.getElementById('rp-status');
    if(status) status.textContent=t('replayDone');
    stopReplay();
    return;
  }
  if(replayPaused) return;
  const s=replayData.sentences[replayIdx];
  // Send sentence to NMEA bus via SignalK (or just log it)
  // For now: emit via API
  replayIdx++;
  // Update UI
  const pct=Math.round(replayIdx/replayData.totalSentences*100);
  const progress=document.getElementById('rp-progress');
  const status=document.getElementById('rp-status');
  if(progress) progress.style.width=pct+'%';
  if(status) status.textContent=`${replayIdx} / ${replayData.totalSentences} ${t('replaySentences')} (${replaySpeed}×)`;
  // Calculate delay based on timestamps
  let delayMs=100;
  if(replayIdx<replayData.sentences.length&&s.time&&replayData.sentences[replayIdx].time){
    const t1=parseReplayTime(s.time);
    const t2=parseReplayTime(replayData.sentences[replayIdx].time);
    if(t1!==null&&t2!==null){
      delayMs=Math.max(10,Math.min(2000,(t2-t1)/replaySpeed));
    }
  }
  replayTimer=setTimeout(replayNext,delayMs);
}

function parseReplayTime(t){
  if(!t) return null;
  const p=t.split(':');
...
function toggleReplayPause(){
  ...
  const pause=document.getElementById('rp-pause');
  if(pause) pause.textContent=replayPaused?'▶':'⏸';
  if(!replayPaused) replayNext();
}

function stopReplay(){
  if(replayTimer){clearTimeout(replayTimer);replayTimer=null;}
  replayData=null;replayIdx=0;replayPaused=false;
  const play=document.getElementById('rp-play');
  const pause=document.getElementById('rp-pause');
  const stop=document.getElementById('rp-stop');
