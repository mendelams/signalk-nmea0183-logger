...
const EVT_ICONS={engine:'⚙️',course:'🧭',wind:'💨',battery:'🔋',hazard:'⚠️',sighting:'🐬',vhf:'📻',dsc:'📡',note:'📝',custom:'📌',anchor:'⚓',harbor:'🏁',sailchange:'⛵'};
function evIcon(ev){
  if(ev.type==='engine'){
    if(ev.engineState==='on') return '▶️';
    if(ev.engineState==='off') return '⏹️';
  }
  return EVT_ICONS[ev.type]||'📌';
}

// ── Helpers ───────────────────────────────────────
/** Base URL for all API calls (same origin as the web UI). */
const API_BASE=window.location.origin; // e.g. http://openplotter.local:3033
// ── JSON parsing worker (off-loads big responses from main thread) ──
let _worker=null, _workerSeq=0;
const _workerCallbacks={};
function getWorker(){
  if(_worker) return _worker;
  try{
    _worker=new Worker('/parse-worker.js');
    _worker.onmessage=e=>{
      const cb=_workerCallbacks[e.data.id];
      if(!cb) return;
      delete _workerCallbacks[e.data.id];
      if(e.data.error) cb.reject(new Error(e.data.error));
      else cb.resolve(e.data.data);
    };
    _worker.onerror=()=>_worker=null;
  }catch(e){ /* Worker not available — fall back to main thread */ }
  return _worker;
}
function parseJsonOffthread(text){
  // Only worth the worker round-trip for large payloads
  if(text.length<50000) return Promise.resolve(JSON.parse(text));
  const w=getWorker();
  if(!w) return Promise.resolve(JSON.parse(text));
...
async function api(e){
  ...
  // Use worker for big responses (stats with track data etc.)
  const text=await r.text();
  return parseJsonOffthread(text);
}
...
const PREF_DEFAULTS={
  ...
  gpxAis:false, gpxEvents:true, dlRaw:false, polarStrict:false
};
...
function renderTabs(){
  return `<div class="tabs">
    ...
    <div class="tab ${activeTab==='crew'?'active':''}" onclick="activeTab='crew';showCrew()">${t('crewTab')}</div>
...
/** Render the main list view (logs, voyages, or season depending on active tab). */
    ...
    if(activeTab==='voyages'){
      ...
      if(!files.length){h+=`<div class="card"><div class="empty">${t('noLogs')}</div></div>`;}
        ...
        for(const f of files){
          ...
          h+=`<td class="fsize">${fmtSize(f.size)}${f.gz?'<span class="gz-badge" title="Compressed on disk; download size will be larger">gz</span>':''}</td>`;
...
/** Render the engine/fuel/maintenance tab with live tank data and schedules. */
...
async function saveFuel(){
  ...
  if(!dateVal||!/^\d{4}-\d{2}-\d{2}$/.test(dateVal)){toast(t('fuelDate')+'?','err');return;}
  const liters=parseFloat(document.getElementById('ef-liters').value);
  if(!liters||liters<=0||liters>9999){toast('Liters?','err');return;}
  const hours=parseFloat(document.getElementById('ef-hours').value);
  if(isNaN(hours)||hours<0){toast(t('fuelHours')+'?','err');return;}
  ...
  const data={
    ...
    hours,
    liters,
    pricePerLiter:isNaN(price)||price<0?null:price,
    totalCost:isNaN(price)||price<0?null:Math.round(liters*price*100)/100,
    fullTank:document.getElementById('ef-full').checked,
    note:document.getElementById('ef-note').value.trim().substring(0,500)
  };
  try{
    if(editId){
...
async function saveMaint(){
  ...
  if(!dateVal||!/^\d{4}-\d{2}-\d{2}$/.test(dateVal)){toast(t('fuelDate')+'?','err');return;}
  const hours=parseFloat(document.getElementById('em-hours').value);
  if(isNaN(hours)||hours<0){toast(t('fuelHours')+'?','err');return;}
  ...
  const data={
    ...
    note:document.getElementById('em-note').value.trim().substring(0,500)
  };
  try{
    if(editId){
...
/* ════════════════════════════════════════════════════
   POWER TAB
   ════════════════════════════════════════════════════ */
/** Render the power/electrical tab with live battery and charger data. */
...
async function viewVoyage(vid){return guard(async()=>{
  ...
  <div class="export-panel" id="export-panel">
    ...
    <span class="export-close" onclick="toggleExport()" title="${t('close')||'Close'}">✕</span>
  </div>`;
  ...
  if(v.track&&v.track.length>0){
    ...
    leafletMap=L.map('map',{zoomControl:true,attributionControl:false,dragging:!L.Browser.mobile,tap:false});
...
async function viewFile(fn){return guard(async()=>{
  ...
  <div class="export-panel" id="export-panel">
    <div class="export-btns">
      ...
      <a class="btn btn-sm" id="exp-download" href="/api/logs/${fn}/download">${t('download')}</a>
      <span class="btn btn-sm" onclick="printReport()">🖨</span>
      <a class="btn btn-sm" href="/api/logs/${fn}/report" target="_blank">📋 ${t('reportGenerate')}</a>
      <span class="btn-polar-group">
        <a class="btn btn-sm btn-blue" id="exp-polar" href="/api/logs/${fn}/clean" title="${t('polarHint')||'Download without AIS and engine periods'}">⛵ ${t('polarDownload')||'Polar'}</a><label class="btn-polar-strict" title="${t('polarStrictHint')||'Keep only polar-essential sentences'}"><input type="checkbox" id="exp-strict" ${P.polarStrict?'checked':''} onchange="setSave('polarStrict',this.checked);updatePolarUrl('${fn}')">${t('polarStrict')||'Strict'}</label><span class="btn btn-sm btn-blue" style="border-left:1px solid rgba(255,255,255,.2);border-radius:0 6px 6px 0" onclick="savePolarToServer('${fn}')" title="${t('polarSaveHint')||'Save to polar_logs folder on server'}">💾</span>
      </span>
    </div>
    <div class="export-opts">
      <label><input type="checkbox" id="exp-ais" ${P.gpxAis?'checked':''} onchange="setSave('gpxAis',this.checked);updateGpxUrl()"> AIS</label>
      <label><input type="checkbox" id="exp-ev" ${P.gpxEvents!==false?'checked':''} onchange="setSave('gpxEvents',this.checked);updateGpxUrl()"> Events</label>
      <label title="${t('rawHint')||'Strip ISO timestamps from download (e.g. for OpenCPN VDR replay)'}"><input type="checkbox" id="exp-raw" ${P.dlRaw?'checked':''} onchange="setSave('dlRaw',this.checked);updateDownloadUrl('${fn}')"> ${t('rawDownload')||'Raw'}</label>
    </div>
    <span class="export-close" onclick="toggleExport()" title="${t('close')||'Close'}">✕</span>
  </div>`;
  if(inVoyages.length){
  ...
  setTimeout(()=>{updateGpxUrl();updateDownloadUrl(fn);updatePolarUrl(fn);},0);

  if(stats&&stats.track&&stats.track.length>0){
    h+='<div class="card map-card" id="map-card"><div class="map-toggle-btn" onclick="toggleMapExpand()" title="Expand / collapse map">⤢</div><div id="map"></div></div>';
  } else {
  ...
  if(stats){
    ...
    if(stats.vdoTrackPoints>0){
      h+=sg(t('vdoFallback')||'VDO track pts',stats.vdoTrackPoints,'');
    }
    if(stats.manualEngineHours>0){
      h+=sg(t('hoursManual')||'Manual',fmtDur(stats.manualEngineHours),'');
    }
    // Engine sensor readings from ESP32 monitor (XDR sentences)
    if(stats.engineTempSamples>0){
      h+=sg(t('engineTempAvg')||'Engine temp',stats.engineTempAvg,'°C');
      h+=sg(t('engineTempMax')||'Max temp',stats.engineTempMax,'°C');
    }
    if(stats.fuelLevelSamples>0){
      h+=sg(t('fuelLevelStart')||'Fuel start',stats.fuelLevelStart,'%');
      h+=sg(t('fuelLevelEnd')||'Fuel end',stats.fuelLevelEnd,'%');
    }
    if(stats.runtimeEnd!==null&&stats.runtimeEnd!==undefined){
      h+=sg(t('runtimeMeter')||'Hour meter',stats.runtimeEnd.toFixed(2),'h');
    }
    ...
    if(stats.fuelSegments){
      ...
      h+=sg(t('fuelMotorDist'),fs.motorDistNm,'nm');
      h+=sg(t('fuelSailDist'),fs.sailDistNm,'nm');
      h+=sg(t('fuelMotorPct'),fs.motorPct,'%');
      if(fs.estimatedLiters) h+=sg(t('fuelLperNm'),(fs.estimatedLiters/(fs.motorDistNm||1)).toFixed(2),'');
    }
    ...
    // ── Charts section (after autopilot stats) ────────────────────
    h+=renderMiniCharts(stats,fn);
  }
  ...
  <div class="ev-form hidden" id="ev-form">
    <div class="ev-form-row">
      <select id="ev-type" onchange="onEventTypeChange()">
        ...
        <option value="engine">🔧 ${t('evTypeEngine')||'Engine'}</option>
        <option value="custom">📌 ${t('evTypeCustom')}</option>
      </select>
      <input type="time" id="ev-time" step="60">
    </div>
    <div class="ev-form-row hidden" id="ev-engine-row">
      <div class="engine-state-toggle">
        <label><input type="radio" name="ev-engine-state-radio" value="on" checked> ▶ ${t('engineStarted')}</label>
        <label><input type="radio" name="ev-engine-state-radio" value="off"> ■ ${t('engineStopped')}</label>
      </div>
      <input type="number" id="ev-engine-hours" placeholder="${t('hourMeter')||'Hour meter'}" step="0.1" min="0">
    </div>
...
/** Render a stat grid item with label, value, and optional unit. */
...
/** Open a card container with optional title. */
...
/** Render an orange warning card with title and optional detail text. */
...
/** Toggle map card between default height and 1:1 square (full width). */
function toggleMapExpand(){
  const card=document.getElementById('map-card');
  const btn=card?card.querySelector('.map-toggle-btn'):null;
  if(!card) return;
  const expanded=card.classList.toggle('map-expanded');
  if(btn) btn.textContent=expanded?'⤡':'⤢';
  // Leaflet needs an invalidateSize after a container resize
  if(leafletMap) setTimeout(()=>leafletMap.invalidateSize(),200);
}

/** Render Leaflet map with GPS track, events, shallowest point, and optional SOG heatmap. */
...
/** Update the download link based on Raw checkbox state. */
function updateDownloadUrl(fn){
  const el=document.getElementById('exp-download');
  const raw=document.getElementById('exp-raw');
  if(!el) return;
  const base='/api/logs/'+encodeURIComponent(fn)+'/download';
  el.href=raw&&raw.checked?base+'?raw=1':base;
}
...
/**
 * Render SVG polyline chart from a data series.
 * @param {Array} s1 - Primary series [{t, v}]
 * @param {Array} s2 - Secondary series [{t, v}] (optional)
 * @param {string} c1 - Color for series 1
 * @param {string} c2 - Color for series 2
 * @param {number} w - Width
 * @param {number} h - Height
 * @param {boolean} invertY - Invert Y axis (for depth: deeper = lower on screen)
 */
function svgChart(s1, s2, c1, c2, w, h, invertY, symmetric) {
  if (!s1 || !s1.length) return `<svg viewBox="0 0 ${w} ${h}" class="chart-canvas"><text x="${w/2}" y="${h/2}" text-anchor="middle" font-size="10" fill="var(--muted)">No data</text></svg>`;
  const pad = 4;
  const ew = w - pad * 2, eh = h - pad * 2;

  // Clip both series to shared time window
  const tStart1 = new Date(s1[0].t).getTime();
  const tEnd1   = new Date(s1[s1.length-1].t).getTime();
  const tStart2 = s2 && s2.length ? new Date(s2[0].t).getTime() : tStart1;
  const tEnd2   = s2 && s2.length ? new Date(s2[s2.length-1].t).getTime() : tEnd1;
  const tMinFinal = Math.max(tStart1, tStart2);
  const tMaxFinal = Math.min(tEnd1, tEnd2);
  const useShared = tMaxFinal > tMinFinal;
  const tF = useShared ? tMinFinal : tStart1;
  const tT = useShared ? tMaxFinal : tEnd1;
  const tRange = tT - tF || 1;

  const clip1 = useShared ? s1.filter(p => { const t = new Date(p.t).getTime(); return t >= tF && t <= tT; }) : s1;
  const clip2 = useShared && s2 ? s2.filter(p => { const t = new Date(p.t).getTime(); return t >= tF && t <= tT; }) : (s2 || []);

  const all = [...clip1, ...clip2];
  if (!all.length) return `<svg viewBox="0 0 ${w} ${h}" class="chart-canvas"><text x="${w/2}" y="${h/2}" text-anchor="middle" font-size="10" fill="var(--muted)">No data</text></svg>`;
  const allV = all.map(p => p.v);
  let minV = Math.min(...allV), maxV = Math.max(...allV);

  // Symmetric: force Y-axis to be symmetric around 0 (for wind angle charts)
  if (symmetric) {
    const absMax = Math.max(Math.abs(minV), Math.abs(maxV), 30); // min ±30°
  ...
  function toPoints(series) {
    ...
    return series.map(p => {
      const x = pad + ((new Date(p.t).getTime() - tF) / tRange) * ew;
      const yRaw = (p.v - minV) / range;
      const y = invertY ? pad + yRaw * eh : pad + (1 - yRaw) * eh;
      return `${x.toFixed(1)},${y.toFixed(1)}`;
    }).join(' ');
  }

  // Zero line for symmetric charts — theme-adaptive, drawn before data
  let extras = '';
  if (symmetric) {
    const yZero = pad + (1 - (0 - minV) / range) * eh;
    // SB (green) bar: from top to zero, 2px on left edge
    extras += `<rect x="${pad-2}" y="${pad}" width="2" height="${(yZero-pad).toFixed(1)}" fill="rgba(76,175,80,.6)"/>`;
    // BB (red) bar: from zero to bottom
    extras += `<rect x="${pad-2}" y="${yZero.toFixed(1)}" width="2" height="${(pad+eh-yZero).toFixed(1)}" fill="rgba(229,57,53,.6)"/>`;
    // Zero line — use currentColor so it adapts to SVG context
    extras += `<line x1="${pad}" y1="${yZero.toFixed(1)}" x2="${pad+ew}" y2="${yZero.toFixed(1)}" stroke="currentColor" stroke-opacity=".4" stroke-width="1.5"/>`;
  }

  let paths = extras;
  paths += `<polyline points="${toPoints(clip1)}" fill="none" stroke="${c1}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"/>`;
  if (clip2 && clip2.length) {
    paths += `<polyline points="${toPoints(clip2)}" fill="none" stroke="${c2}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round" stroke-dasharray="3,2"/>`;
  }
  return `<svg viewBox="0 0 ${w} ${h}" class="chart-canvas" preserveAspectRatio="none">${paths}</svg>`;
}

/** Render the four mini chart cards below the navigation stats. */
function renderMiniCharts(stats, fn) {
  if (!stats) return '';
  const s = stats;
  const hasSeries = s => s && s.length > 1;
  const charts = [
    {
      id: 'wind-speed', label: t('wind')||'Wind',
      s1: s.twsSeries, s2: s.awsSeries,
      c1: 'var(--blue)', c2: 'var(--orange)',
      l1: 'TWS', l2: 'AWS', invertY: false,
      unit: uWind()
    },
    {
      id: 'wind-angle', label: t('windAngle')||'Wind angle',
      s1: s.twaSeries, s2: s.awaSeries,
      c1: 'var(--purple)', c2: 'var(--cyan,#26c6da)',
      l1: 'TWA', l2: 'AWA', invertY: false, symmetric: true,
      unit: '°'
    },
    {
      id: 'speed', label: t('speed')||'Speed',
      s1: s.sogSeries, s2: s.stwSeries,
      c1: 'var(--green)', c2: 'var(--orange)',
      l1: 'SOG', l2: 'STW', invertY: false,
      unit: uSpeed()
    },
    {
      id: 'depth', label: t('depth')||'Depth',
      s1: s.depthSeries, s2: null,
      c1: 'var(--cyan,#26c6da)', c2: null,
      l1: t('depth')||'Depth', l2: null, invertY: true,
      unit: 'm'
    }
  ];

  let h = `<div class="card"><div class="card-title">📈 ${t('charts')||'Grafieken'}</div><div class="charts-row">`;
  for (const ch of charts) {
    const hasData = hasSeries(ch.s1);
    const onclick = hasData ? `onclick="openFullChart('${fn}','${ch.id}')"` : '';
    const svg = svgChart(ch.s1, ch.s2, ch.c1, ch.c2, 200, 80, ch.invertY, ch.symmetric);
    const legend = `<div class="chart-legend">
      <span class="chart-legend-item"><span class="chart-legend-dot" style="background:${ch.c1}"></span>${ch.l1}</span>
      ${ch.l2 && hasSeries(ch.s2) ? `<span class="chart-legend-item"><span class="chart-legend-dot" style="background:${ch.c2};border:1px dashed ${ch.c2}"></span>${ch.l2}</span>` : ''}
    </div>`;
    h += `<div class="chart-card" ${onclick} title="${hasData ? 'Click to expand' : 'No data'}">
      <div class="chart-title">${ch.label}</div>
      ${svg}
      ${legend}
    </div>`;
  }
...
/** Open a full-screen interactive chart overlay.
 *  - Fetches high-res series from /api/logs/:fn/series?type=...
 *  - Canvas rendering with touch/mouse scrubbing and tooltip
 *  - Dropdown switcher between chart types
 *  - Back button removes overlay (no page navigation)
 */
function openFullChart(fn, chartId) {
  // Chart type definitions
  const CHART_TYPES = [
    { id: 'wind-speed', label: t('wind')||'Wind speed',  l1:'TWS', l2:'AWS', unit:'kn', c1:'#4fc3f7', c2:'#ff9800' },
    { id: 'wind-angle', label: t('windAngle')||'Wind angle', l1:'TWA', l2:'AWA', unit:'°', c1:'#ce93d8', c2:'#26c6da', symmetric: true },
    { id: 'speed',      label: t('speed')||'Speed',      l1:'SOG', l2:'STW', unit:'kn', c1:'#66bb6a', c2:'#ff9800' },
    { id: 'depth',      label: t('depth')||'Depth',      l1:'Depth', l2:null, unit:'m', c1:'#26c6da', c2:null, invertY:true }
  ];
  ...
  overlay.innerHTML = `
    <div class="chart-overlay-header">
      <span class="btn btn-sm" id="chart-back">← ${t('back')||'Back'}</span>
      <select id="chart-type-select" class="chart-type-select">
        ${CHART_TYPES.map(c => `<option value="${c.id}"${c.id===chartId?' selected':''}>${c.label}</option>`).join('')}
      </select>
      <div class="chart-legend" id="chart-legend-full"></div>
    </div>
    <div class="chart-wrap">
      <canvas id="chart-canvas-full"></canvas>
  ...
  let activeType = chartId;
  let animFrame = null;

  // Back button
  overlay.querySelector('#chart-back').addEventListener('click', () => {
    overlay.remove();
  });

  // Type switcher — no page refresh, just reload data
  overlay.querySelector('#chart-type-select').addEventListener('change', e => {
    activeType = e.target.value;
    loadAndDraw(activeType);
  });

  // Load series data and draw
  async function loadAndDraw(type) {
    const statusEl = overlay.querySelector('#chart-status');
    statusEl.textContent = 'Loading…';
    statusEl.style.display = '';
    try {
      const data = await api('/api/logs/' + encodeURIComponent(fn) + '/series?type=' + type);
      currentData = data;
      statusEl.style.display = 'none';
      const def = CHART_TYPES.find(c => c.id === type) || CHART_TYPES[0];
      // Update legend
      const leg = overlay.querySelector('#chart-legend-full');
      leg.innerHTML = `
        <span class="chart-legend-item"><span class="chart-legend-dot" style="background:${def.c1}"></span>${data.label1} ${data.unit}</span>
        ${data.label2 ? `<span class="chart-legend-item"><span class="chart-legend-dot" style="background:${def.c2}"></span>${data.label2} ${data.unit}</span>` : ''}
      `;
      draw(data, def, null);
    } catch(e) {
  ...
  // ── Canvas drawing ───────────────────────────────────────────────
  function draw(data, def, scrubX) {
    const canvas = overlay.querySelector('#chart-canvas-full');
    const wrap   = overlay.querySelector('.chart-wrap');
    ...
    // Symmetric Y-axis: force range to be symmetric around 0 (wind angle charts)
    if (def.symmetric) {
    ...
    // Time range
    const tMin = new Date(all[0].t).getTime();
    const tMax = new Date(all[all.length-1].t).getTime();
    const tRange = tMax - tMin || 1;

    function xOf(t)  { return padL + ((new Date(t).getTime() - tMin) / tRange) * pw; }
    function yOf(v)  {
      const r = (v - minV) / rangeV;
      return def.invertY ? padT + r * ph : padT + (1 - r) * ph;
    }

    // Background
    ctx.fillStyle = getComputedStyle(document.body).getPropertyValue('--bg') || '#121212';
    ...
    // Zero line for symmetric charts (wind angle)
    if (def.symmetric && minV < 0 && maxV > 0) {
      const yZero = yOf(0);
      const yTop  = yOf(maxV);  // SB top (positive = starboard = above zero)
      const yBot  = yOf(minV);  // BB bottom (negative = port = below zero)

      // Theme-adaptive text color
      const textColor = getComputedStyle(document.body).getPropertyValue('--text').trim() || '#e0e0e0';
      const mutedColor = getComputedStyle(document.body).getPropertyValue('--muted').trim() || '#888';

      // ── SB/BB color bar on Y-axis (2px wide) ──
      // Green = SB (positive, above zero line)
      ctx.fillStyle = 'rgba(76,175,80,.7)';   // green
      ctx.fillRect(padL - 3, yTop, 2, yZero - yTop);
      // Red = BB (negative, below zero line)
      ctx.fillStyle = 'rgba(229,57,53,.7)';   // red
      ctx.fillRect(padL - 3, yZero, 2, yBot - yZero);

      // ── Zero line — theme-adaptive ──
      ...
      // SB / BB labels next to bar
      ctx.font = '10px sans-serif'; ctx.textAlign = 'right';
      ctx.fillStyle = 'rgba(76,175,80,.8)';
      ctx.fillText('SB', padL - 6, yTop + 14);
      ctx.fillStyle = 'rgba(229,57,53,.8)';
      ctx.fillText('BB', padL - 6, yBot - 4);
    }
    // Time axis
    const nTime = Math.min(6, Math.floor(pw / 60));
    for (let i = 0; i <= nTime; i++) {
      const ts = new Date(tMin + (tRange / nTime) * i);
      const x = padL + (i / nTime) * pw;
    ...
    // Draw series
    function drawSeries(series, color, dashed) {
      if (!series || !series.length) return;
      ctx.beginPath();
    ...
    drawSeries(data.s1, def.c1, false);
    drawSeries(data.s2, def.c2, true);

    // Scrub line + tooltip
    if (scrubX !== null && scrubX !== undefined) {
      // Find nearest data points
      const xRatio = (scrubX - padL) / pw;
      if (xRatio >= 0 && xRatio <= 1) {
        // Vertical scrub line
        ctx.strokeStyle = 'rgba(255,255,255,.4)';
        ctx.lineWidth = 1;
        ctx.setLineDash([4,3]);
        ctx.beginPath(); ctx.moveTo(scrubX, padT); ctx.lineTo(scrubX, padT + ph); ctx.stroke();
        ctx.setLineDash([]);
        ...
        function nearest(series) {
          ...
          for (const p of series) {
            const dx = Math.abs(xOf(p.t) - scrubX);
            if (dx < bestDx) { bestDx = dx; best = p; }
          }
          return best;
        }
        const p1 = nearest(data.s1);
        const p2 = nearest(data.s2);

        // Draw dots at nearest points
        if (p1) {
          ctx.beginPath(); ctx.arc(xOf(p1.t), yOf(p1.v), 5, 0, Math.PI*2);
          ctx.fillStyle = def.c1; ctx.fill();
        }
        if (p2) {
          ctx.beginPath(); ctx.arc(xOf(p2.t), yOf(p2.v), 5, 0, Math.PI*2);
          ctx.fillStyle = def.c2; ctx.fill();
        }

        // Update tooltip
        const tooltip = overlay.querySelector('#chart-tooltip');
        const time = p1 ? new Date(p1.t) : null;
        ...
        tooltip.innerHTML = `
          ...
          ${p1 ? `<div class="tooltip-val" style="color:${def.c1}">${def.l1}: <b>${p1.v} ${data.unit}</b></div>` : ''}
          ${p2 ? `<div class="tooltip-val" style="color:${def.c2}">${def.l2}: <b>${p2.v} ${data.unit}</b></div>` : ''}
        `;
        tooltip.classList.remove('hidden');
        // Position tooltip: left or right of scrub line
        const tipW = 140;
        const tipLeft = scrubX + tipW + 20 > W ? scrubX - tipW - 10 : scrubX + 10;
        tooltip.style.left = tipLeft + 'px';
        tooltip.style.top = padT + 'px';
      }
    } else {
      overlay.querySelector('#chart-tooltip').classList.add('hidden');
    }
  }
  ...
  function handlePointer(e) {
    ...
    const def = CHART_TYPES.find(c => c.id === activeType) || CHART_TYPES[0];
    if (animFrame) cancelAnimationFrame(animFrame);
    animFrame = requestAnimationFrame(() => draw(currentData, def, x));
    e.preventDefault();
  }
  function handlePointerEnd() {
    ...
    const def = CHART_TYPES.find(c => c.id === activeType) || CHART_TYPES[0];
    if (animFrame) cancelAnimationFrame(animFrame);
    animFrame = requestAnimationFrame(() => draw(currentData, def, null));
  }
...
/** Update the polar download link based on Strict checkbox state. */
function updatePolarUrl(fn){
  const el=document.getElementById('exp-polar');
  const strict=document.getElementById('exp-strict');
  if(!el) return;
  const base='/api/logs/'+encodeURIComponent(fn)+'/clean';
  el.href=strict&&strict.checked?base+'?strict=1':base;
}

/** Save polar-cleaned log to polar_logs folder on the server. */
async function savePolarToServer(fn){
  const strict=document.getElementById('exp-strict');
  const params='save=1'+(strict&&strict.checked?'&strict=1':'');
  const url=API_BASE+'/api/logs/'+encodeURIComponent(fn)+'/clean?'+params;
  toast('Saving…','info',10000);
  try{
    const r=await fetch(url);
    const d=await r.json();
    if(d.ok){
      toast(`💾 ${d.file} — ${d.sizeKB}KB`,'ok',4000);
    } else {
      toast('Error: '+(d.error||'unknown'),'err',6000);
    }
  }catch(e){toast('Error: '+e.message,'err',6000);}
}
...
function showAddEvent(fn){
  ...
  const hrsEl=document.getElementById('ev-engine-hours');
...
/** Read selected engine state from radio group. */
function getEngineStateValue(){
  const checked=document.querySelector('input[name="ev-engine-state-radio"]:checked');
  return checked?checked.value:'on';
}
function setEngineStateValue(v){
  const el=document.querySelector(`input[name="ev-engine-state-radio"][value="${v}"]`);
  if(el) el.checked=true;
}

/** Show/hide engine-specific fields based on selected event type. */
function onEventTypeChange(){
  const type=document.getElementById('ev-type').value;
  const row=document.getElementById('ev-engine-row');
  if(!row) return;
  if(type==='engine'){
    row.classList.remove('hidden');
    const hrsEl=document.getElementById('ev-engine-hours');
    // Pre-fill hour meter with current total as a starting suggestion.
    // Only fill if field is empty (don't overwrite user input).
    if(hrsEl&&hrsEl.value===''){
      api('/api/engine').then(eng=>{
        if(eng&&eng.totalHours!==undefined&&hrsEl.value==='') hrsEl.value=eng.totalHours.toFixed(1);
      }).catch(()=>{});
    }
    // Smart default: if last engine event was 'on', default to 'off' (and vice versa)
    if(window._currentLogFn){
      api('/api/logs/'+encodeURIComponent(window._currentLogFn)+'/events').then(ef=>{
        const engEvents=(ef.manualEvents||[]).filter(e=>e.type==='engine'&&e.engineState);
        if(engEvents.length){
          engEvents.sort((a,b)=>(a.time||'').localeCompare(b.time||''));
          const last=engEvents[engEvents.length-1];
          setEngineStateValue(last.engineState==='on'?'off':'on');
        } else {
          setEngineStateValue('on');
        }
      }).catch(()=>{});
    }
  } else {
...
function showEditEvent(fn,type,time,detail,note){
  ...
  // Convert stored UTC time to local-time for the time input
  if(time){
    const d=new Date(time);
    if(!isNaN(d.getTime())){
      const hh=String(d.getHours()).padStart(2,'0');
      const mm=String(d.getMinutes()).padStart(2,'0');
      document.getElementById('ev-time').value=hh+':'+mm;
    } else {
  ...
  // Pre-fill engine fields if this is an engine event (load from manualEvents)
  if(type==='engine'){
    api('/api/logs/'+encodeURIComponent(fn)+'/events').then(ef=>{
      const ev=(ef.manualEvents||[]).find(e=>e.type==='engine'&&e.time===time);
      if(ev){
        if(ev.engineState) setEngineStateValue(ev.engineState);
        const hoursEl=document.getElementById('ev-engine-hours');
        if(hoursEl) hoursEl.value=ev.engineHours!==undefined?ev.engineHours:'';
      }
    }).catch(()=>{});
  }
  onEventTypeChange();
...
async function updateEvent(fn,oldType,oldTime){
  ...
  if(!detail&&type!=='engine'){toast(t('evDetail'),'err');document.getElementById('ev-detail').focus();return;}
  const tp=(timeInput||'').match(/(\d{1,2}):(\d{2})/);
  const hh=tp?parseInt(tp[1]):12;
  const mm=tp?parseInt(tp[2]):0;
  // Local-time to UTC conversion (same as saveEvent)
  const [yr,mo,da]=currentLogDate.split('-').map(Number);
  ...
  try{
    ...
    const engState=getEngineStateValue();
    const payload={action:'add',type,detail:detail||(type==='engine'?(engState==='on'?t('engineStarted'):t('engineStopped')):''),time:newTime,note:note||undefined};
    if(type==='engine'){
      payload.engineState=engState;
      const hoursEl=document.getElementById('ev-engine-hours');
      if(hoursEl&&hoursEl.value!==''){
        const h=parseFloat(hoursEl.value);
        if(!isNaN(h)&&h>=0) payload.engineHours=h;
      }
    }
...
function hideAddEvent(){document.getElementById('ev-form').classList.add('hidden');}
...
async function saveEvent(fn){
  ...
  try{
    ...
    const hh=tp?parseInt(tp[1]):12;
    const mm=tp?parseInt(tp[2]):0;
    // Interpret user-entered time as LOCAL time (matches the time picker behavior).
    // Build a Date using local-tz components, then convert to UTC ISO for storage.
    const [yr,mo,da]=currentLogDate.split('-').map(Number);
    const d=new Date(yr,mo-1,da,hh,mm,0,0);
    if(isNaN(d.getTime())){toast('Invalid date','err',8000);return;}
    const evTime=d.toISOString();
    const payload={action:'add',type,detail,time:evTime,note:note||undefined};
    // Engine event: include state and optional hour-meter reading
    if(type==='engine'){
      const engState=getEngineStateValue();
      payload.engineState=engState;
      const hoursEl=document.getElementById('ev-engine-hours');
      ...
      // Auto-fill detail if user left it blank
      if(!detail) payload.detail=engState==='on'?t('engineStarted'):t('engineStopped');
    }
    const url=API_BASE+'/api/logs/'+encodeURIComponent(fn)+'/events';
    ...
    h+=`<div class="card"><div class="set-group">
      ...
      <div class="set-row" style="border-bottom:none;padding-bottom:0">
        ...
        <div style="display:flex;gap:4px;align-items:center;font-size:.68em;color:var(--muted);text-transform:uppercase;font-weight:600">
          <span style="width:72px;text-align:center">${t('durH')}</span>
          <span style="width:20px;text-align:center">${t('or')||'of'}</span>
          <span style="width:72px;text-align:center">mnd</span>
        </div>
      </div>`;
    for(let i=0;i<(eng.schedule||[]).length;i++){
      ...
      h+=`<div class="set-row">
        ...
        <div style="display:flex;gap:4px;align-items:center">
          <input type="number" class="set-select maint-interval-input" id="ms-h-${i}" value="${s.intervalHours||''}" placeholder="—">
          <span class="tiny-text">${t('or')||'of'}</span>
          <input type="number" class="set-select maint-interval-input" id="ms-m-${i}" value="${s.intervalMonths||''}" placeholder="—">
        </div>
      </div>`;
    }
...
/* ════════════════════════════════════════════════════
   CREW TAB
   ════════════════════════════════════════════════════ */
const CREW_ROLES=['skipper','mate','crew','guest'];
const CREW_ICONS={skipper:'👨‍✈️',mate:'🧭',crew:'⛵',guest:'🎫'};

/** Render the crew management tab with member list and add/edit forms. */
async function showCrew(){
  stopAuto();destroyMap();
  currentView={type:'list'};
  if(!_fromPop) history.pushState({type:'list',tab:'crew'},'','#list');
  const el=document.getElementById('app');
  el.innerHTML=renderTabs()+`<div class="loading">${t('loading')}</div>`;
  let crew;
  try{crew=await api('/api/crew');}catch(e){el.innerHTML=renderTabs()+`<div class="card"><div class="empty">${t('errorPrefix')}: ${esc(e.message)}</div></div>`;return;}
  let h=renderTabs();
  h+=`<div class="card"><div class="card-title">${t('crewList')}</div>`;
  if(crew.members.length){
    for(const m of crew.members){
      const icon=CREW_ICONS[m.role]||'⛵';
      const roleLabel=t('crewRole'+m.role.charAt(0).toUpperCase()+m.role.slice(1))||m.role;
      const dateRange=m.from?(m.from+(m.to?' → '+m.to:'')):'';
      h+=`<div class="hist-item" id="crew-${m.id}">
        <span class="hist-icon">${icon}</span>
        <span class="flex-1"><b>${esc(m.name)}</b> — ${roleLabel}${m.cert?' <span class="meta-text">('+esc(m.cert)+')</span>':''}${dateRange?' <span class="tiny-text">'+dateRange+'</span>':''}</span>
        <span class="ev-note-btn" onclick="editCrew('${m.id}')">✏️</span>
        <span class="ev-del action-btn" onclick="deleteCrew('${m.id}')">✕</span>
      </div>`;
    }
  } else {
    h+=`<div class="empty">${t('crewNoMembers')}</div>`;
  }
  h+=addBar(t('crewAdd'),"toggleEngForm('crew-form')");
  h+=`<div class="eng-form hidden" id="crew-form">
    <input type="hidden" id="crew-edit-id" value="">
    <div class="eng-form-row">
      <div><label class="eng-field-label">${t('crewName')}</label><input type="text" id="cr-name" placeholder="${t('crewName')}"></div>
      <div><label class="eng-field-label">${t('crewRole')}</label><select id="cr-role">
        ${CREW_ROLES.map(r=>`<option value="${r}">${CREW_ICONS[r]} ${t('crewRole'+r.charAt(0).toUpperCase()+r.slice(1))||r}</option>`).join('')}
      </select></div>
    </div>
    <div class="eng-form-row">
      <div><label class="eng-field-label">${t('crewCert')}</label><input type="text" id="cr-cert" placeholder="${t('crewCert')}"></div>
    </div>
    <div class="eng-form-row">
      <div><label class="eng-field-label">${t('crewFrom')}</label><input type="date" id="cr-from"></div>
      <div><label class="eng-field-label">${t('crewTo')}</label><input type="date" id="cr-to"></div>
    </div>
    <div class="eng-form-row">
      <span class="btn btn-sm btn-blue" onclick="saveCrew()">✓</span>
      <span class="btn btn-sm" onclick="cancelCrewEdit()">✕</span>
    </div>
  </div></div>`;
  el.innerHTML=h;
  loadChips();
...
async function saveCrew(){
  const name=document.getElementById('cr-name').value.trim().substring(0,200);
  if(!name){toast(t('crewName')+'?','err');return;}
  const from=document.getElementById('cr-from').value;
  const to=document.getElementById('cr-to').value;
  if(from&&!/^\d{4}-\d{2}-\d{2}$/.test(from)){toast(t('crewFrom')+'?','err');return;}
  if(to&&!/^\d{4}-\d{2}-\d{2}$/.test(to)){toast(t('crewTo')+'?','err');return;}
  if(from&&to&&to<from){toast(t('crewTo')+' < '+t('crewFrom'),'err');return;}
  const editId=document.getElementById('crew-edit-id').value;
  const data={
    name,
    role:document.getElementById('cr-role').value,
    cert:document.getElementById('cr-cert').value.trim().substring(0,500),
    from, to
  };
  try{
    if(editId){data.action='update';data.id=editId;}
    else{data.action='add';}
    await postApi('/api/crew',data);
    toast('👤 ✓','ok',2000);showCrew();
  }catch(e){toast('Error: '+e.message,'err');}
}
function editCrew(id){
  api('/api/crew').then(crew=>{
    const m=crew.members.find(x=>x.id===id);
    if(!m) return;
    document.getElementById('crew-edit-id').value=id;
    document.getElementById('cr-name').value=m.name||'';
    document.getElementById('cr-role').value=m.role||'crew';
    document.getElementById('cr-cert').value=m.cert||'';
    document.getElementById('cr-from').value=m.from||'';
    document.getElementById('cr-to').value=m.to||'';
    const form=document.getElementById('crew-form');
    if(form){form.classList.remove('hidden');setTimeout(()=>form.scrollIntoView({behavior:'smooth',block:'nearest'}),50);}
  });
}
function cancelCrewEdit(){
  document.getElementById('crew-edit-id').value='';
  toggleEngForm('crew-form');
}
async function deleteCrew(id){
  if(!confirm('Delete?'))return;
  try{await postApi('/api/crew',{action:'delete',id});showCrew();}catch(e){toast('Error','err');}
}
