...
const LANG = {
  en: {
    ...
    evTypeEngine: 'Engine',
    hourMeter: 'Hour meter (h)',
    hoursManual: 'Manual entry',
    engineTempAvg: 'Engine temp',
    engineTempMax: 'Max temp',
    fuelLevelStart: 'Fuel start',
    fuelLevelEnd: 'Fuel end',
    runtimeMeter: 'Hour meter',
    vdoFallback: 'VDO track pts',
    rawDownload: 'Raw',
    rawHint: 'Strip ISO timestamps (e.g. for OpenCPN VDR replay)',
    polarDownload: 'Polar',
    polarHint: 'Download without AIS and engine periods — for polar building',
    polarStrict: 'Strict',
    polarStrictHint: 'Keep only polar-essential sentences: RMC, MWV, VHW, VWR, HDG, VTG',
    sogAvgMoving: 'Avg moving speed',
    // Replay
  },
  nl: {
    _name: 'Nederlands',
    ...
    evTypeEngine: 'Motor',
    hourMeter: 'Urenteller (u)',
    hoursManual: 'Handmatig',
    engineTempAvg: 'Motortemp',
    engineTempMax: 'Max temp',
    fuelLevelStart: 'Brandstof start',
    fuelLevelEnd: 'Brandstof eind',
    runtimeMeter: 'Urenteller',
    vdoFallback: 'VDO trackpunten',
    rawDownload: 'Raw',
    rawHint: 'Verwijder ISO timestamps (bv. voor OpenCPN VDR replay)',
    polarDownload: 'Polar',
    polarHint: 'Download zonder AIS en motorperiodes — voor polar opbouw',
    polarStrict: 'Strikt',
    polarStrictHint: 'Alleen polar-essentiële zinnen bewaren: RMC, MWV, VHW, VWR, HDG, VTG',
    sogAvgMoving: 'Gem. vaarsnelheid',
    // Replay
  }
};
...
module.exports = function (app) {
  ...
  const { parseLogFile, parseLogFileAsync } = require('./lib/parser');
  ...
  /**
   * Build stats cache for a completed log file in the background.
   * Called after a log file is closed (day rotation or plugin stop).
   * Uses parseLogFileAsync so the event loop stays free during parsing.
   * Silently skips if cache already exists and is newer than the log file.
   */
  function buildStatsCacheInBackground(logFn) {
    const resolved = resolveLog(logFn);
    if (!resolved) return;
    const fp = resolved.path;
    const cachePath = statsCachePath(fp);
    // Skip if cache already fresh
    try {
      if (fs.existsSync(cachePath)) {
        const cacheMtime = fs.statSync(cachePath).mtimeMs;
        const logMtime   = fs.statSync(fp).mtimeMs;
        if (cacheMtime >= logMtime) return; // already up to date
      }
    } catch (e) { /* ignore stat errors */ }

    // Use setImmediate so the log-close path returns first
    setImmediate(() => {
      app.debug(`Building stats cache for ${logFn}…`);
      parseLogFileAsync(fp, config, t, {}).then(data => {
        try {
          const diskData = Object.assign({}, data);
          delete diskData.aisVessels;
          const json = JSON.stringify(diskData);
          fs.writeFileSync(cachePath, json);
          // Also warm the in-memory cache
          const memKey = `${fp}:${fs.statSync(fp).mtimeMs}`;
          statsCache[memKey] = { data, time: Date.now() };
          app.debug(`Stats cache built for ${logFn} (${(json.length/1024).toFixed(0)}KB)`);
        } catch (e) { app.debug(`Stats cache write error for ${logFn}: ${e.message}`); }
      }).catch(e => { app.debug(`Stats cache build failed for ${logFn}: ${e.message}`); });
    });
  }
  ...
  function getWriteStream() {
    ...
    if (needNew || !currentWriteStream) {
      if (currentWriteStream) {
        // Log file is closing — build its stats cache in the background
        const closingDate = currentLogDate;
        if (closingDate) {
          const suffix = currentFilePart > 0 ? `_part${currentFilePart}` : '';
          buildStatsCacheInBackground(`nmea0183_${closingDate}${suffix}.log`);
        }
        currentWriteStream.end(); currentWriteStream = null;
      }
  ...
  function writeEventsFile(logFn, data) {
    ...
    try {
      ...
      // Manual engine events affect total engine hours — invalidate cache
      if (typeof invalidateEngineHoursCache === 'function') invalidateEngineHoursCache();
    } catch (e) {
  ...
  /**
   * Calculate total engine hours across all log files.
   *
   * Strategy:
   * 1. Collect ALL manual engine events from ALL events.json files into one
   *    sorted timeline — this correctly handles day-boundary crossings where
   *    ON is in one logfile and OFF is in the next.
   * 2. Pair ON/OFF events chronologically. If both have engineHours readings,
   *    use the delta (most accurate). Otherwise use time delta.
   * 3. Use engine.json baseHours as the starting point.
   *    engine-hours.json is treated as read-only legacy — if it has a manual
   *    truth anchor newer than all events.json data, that anchor wins.
   *
   * @returns {number} Total engine hours
   */
  function getTotalEngineHours() {
    ...
    const base = eng.config.baseHours || 0;

    // Scan all events.json files — do this via setImmediate to yield between
    // files and avoid blocking the event loop on slow SD cards.
    // We use a cached result for up to ENGINE_HOURS_TTL (2 min by default).
    if (!logDir || !fs.existsSync(logDir)) {
      _engineHoursCache = { value: base, time: now };
      return base;
    }

    let evFiles;
    try { evFiles = fs.readdirSync(logDir).filter(f => f.endsWith('.events.json')); }
    catch (e) { evFiles = []; }
    ...
    for (const f of evFiles) {
      ...
      try {
        const ef = JSON.parse(fs.readFileSync(fp, 'utf8'));
        for (const ev of (ef.manualEvents || [])) {
          if (ev.type === 'engine' && ev.engineState && ev.time) {
            allEngineEvents.push(ev);
          }
        }
    ...
    allEngineEvents.sort((a, b) => a.time.localeCompare(b.time));

    // ── Step 2: pair ON/OFF across the full timeline ───────────────────────
    let accumulatedHours = 0;
    let lastOn = null;
    for (const ev of allEngineEvents) {
      if (ev.engineState === 'on') {
        lastOn = ev;
      } else if (ev.engineState === 'off' && lastOn) {
        // Prefer hour-meter delta if both sides have a reading
        if (ev.engineHours !== undefined && lastOn.engineHours !== undefined &&
            ev.engineHours > lastOn.engineHours) {
          accumulatedHours += ev.engineHours - lastOn.engineHours;
        } else {
          // Fall back to time delta
          const dt = (new Date(ev.time).getTime() - new Date(lastOn.time).getTime()) / 3600000;
          if (dt > 0 && dt < 48) accumulatedHours += dt;
        }
        lastOn = null;
      }
    }

    // ── Step 3: check legacy engine-hours.json for a truth anchor ─────────
    // If it has a manual reading with a higher hours value AND a timestamp
    // after our last events.json event, use it as the authoritative total.
    let result = base + accumulatedHours;
    const ehPath = path.join(logDir, 'engine-hours.json');
    if (fs.existsSync(ehPath)) {
      try {
        const eh = JSON.parse(fs.readFileSync(ehPath, 'utf8'));
        const lastEv = allEngineEvents.length
          ? allEngineEvents[allEngineEvents.length - 1]
          : null;
        const lastEvTime = lastEv ? new Date(lastEv.time).getTime() : 0;
        const manualAnchors = (eh.readings || [])
          .filter(r => r.manual && r.hours > 0)
          .sort((a, b) => a.time.localeCompare(b.time));
        if (manualAnchors.length) {
          const lastAnchor = manualAnchors[manualAnchors.length - 1];
          const anchorTime = new Date(lastAnchor.time).getTime();
          // Use the anchor if it's more recent than our events.json data
          // OR if it gives a higher (more authoritative) reading
          if (anchorTime >= lastEvTime && lastAnchor.hours > result) {
            result = lastAnchor.hours;
          }
        }
      } catch (e) { app.debug(`engine-hours.json read error: ${e.message}`); }
    }
  ...
  /**
   * Invalidate the engine hours cache. Called when manual engine events change.
   */
  function invalidateEngineHoursCache() {
    _engineHoursCache = { value: null, time: 0 };
  }
  ...
  function readBody(req) {
    return new Promise((resolve, reject) => {
      let body = '';
      // Timeout: if body doesn't arrive within 10s, reject to avoid hanging requests
      const timer = setTimeout(() => { req.destroy(); reject(new Error('Body read timeout')); }, 10000);
      req.on('data', c => { body += c; if (body.length > C.MAX_BODY_SIZE) { clearTimeout(timer); req.destroy(); reject(new Error('Too large')); } });
      req.on('end', () => { clearTimeout(timer); try { resolve(JSON.parse(body)); } catch (e) { reject(new Error('Invalid JSON')); } });
  ...
  // In-memory series cache — stores parsed timeseries per log file.
  // Series are expensive to compute (full async parse) but small enough
  // to keep in memory. TTL = 10 minutes; max 10 entries (LRU eviction).
  const _seriesCache = {};
  const SERIES_CACHE_TTL = 10 * 60 * 1000; // 10 min
  const SERIES_CACHE_MAX = 10;

  function getSeriesCache(fp) {
    const entry = _seriesCache[fp];
    if (!entry) return null;
    if (Date.now() - entry.time > SERIES_CACHE_TTL) { delete _seriesCache[fp]; return null; }
    entry.time = Date.now(); // refresh TTL on hit (LRU)
    return entry.data;
  }

  function setSeriesCache(fp, data) {
    const keys = Object.keys(_seriesCache);
    if (keys.length >= SERIES_CACHE_MAX) {
      // Evict oldest
      const oldest = keys.sort((a, b) => _seriesCache[a].time - _seriesCache[b].time)[0];
      delete _seriesCache[oldest];
    }
    _seriesCache[fp] = { data, time: Date.now() };
  }
  ...
  function mergeManualEvents(st, baseFn) {
    ...
    // Process manual engine events to compute additional engine hours and periods.
    // Pair on/off events to compute duration. If an explicit engineHours reading
    // is given on the 'off' event, use it directly (most accurate).
    const manualEngEvents = (ef.manualEvents || []).filter(e => e.type === 'engine');
    if (manualEngEvents.length) {
      manualEngEvents.sort((a, b) => (a.time || '').localeCompare(b.time || ''));
      let extraHours = 0;
      const manualPeriods = [];
      let lastOn = null;
      for (const ev of manualEngEvents) {
        if (ev.engineState === 'on') {
          lastOn = ev;
        } else if (ev.engineState === 'off' && lastOn) {
          let duration = null;
          // Prefer explicit hour-meter reading delta if both ON and OFF have one
          if (ev.engineHours !== undefined && lastOn.engineHours !== undefined) {
            const diff = ev.engineHours - lastOn.engineHours;
            if (diff > 0 && diff < 24) duration = diff;
          } else if (ev.time && lastOn.time) {
            const dt = (new Date(ev.time).getTime() - new Date(lastOn.time).getTime()) / 3600000;
            if (dt > 0 && dt < 24) duration = dt;
          }
          if (duration) {
            extraHours += duration;
            manualPeriods.push({
      ...
      if (extraHours > 0) {
        st.engineHours = Math.round((((st.engineHours || 0) + extraHours)) * 100) / 100;
        st.manualEngineHours = Math.round(extraHours * 100) / 100;
        st.enginePeriods = (st.enginePeriods || []).concat(manualPeriods);
      }
    }
    return st;
  }
  ...
  function startPublicServer(port) {
    const handler = (req, res) => {
      ...
      try {
        ...
        const m = p.match(/^\/api\/logs\/([^/]+?)(?:\/(stats|download|weather|events|gpx|csv|clean|series))?$/);
        if (m) {
          ...
          if (action === 'events') {
            ...
            if (req.method === 'POST') {
              ...
              readBody(req).then(body => {
                ...
                const ef = readEventsFile(baseFn);
                if (body.action === 'add') {
                  // Add manual event: {action:'add', type, detail, time?, note?, engineState?, engineHours?}
                  const VALID_TYPES = ['hazard','sighting','vhf','note','custom','engine'];
                  ...
                  // Engine-specific fields: state (on/off) and reading on engine hour meter
                  if (evType === 'engine') {
                    if (body.engineState === 'on' || body.engineState === 'off') {
                      ev.engineState = body.engineState;
                    }
                    if (body.engineHours !== undefined && body.engineHours !== null && body.engineHours !== '') {
                      const h = parseFloat(body.engineHours);
                      if (!isNaN(h) && h >= 0) ev.engineHours = h;
                    }
                  }
          ...
          if (action === 'series') {
            const chartType = url.searchParams.get('type') || 'speed';
            const MAX_PTS = 1500;

            function pickSeries(raw, max) {
            ...
            function buildResponse(full, chartType) {
              let s1 = [], s2 = [], label1 = '', label2 = '', unit = '';
              switch (chartType) {
                case 'wind-speed':
                  s1 = pickSeries(full.twsSeries, MAX_PTS); s2 = pickSeries(full.awsSeries, MAX_PTS);
                  label1 = 'TWS'; label2 = 'AWS'; unit = 'kn'; break;
                case 'wind-angle':
                  s1 = pickSeries(full.twaSeries, MAX_PTS); s2 = pickSeries(full.awaSeries, MAX_PTS);
                  label1 = 'TWA'; label2 = 'AWA'; unit = '°'; break;
                case 'speed':
                  s1 = pickSeries(full.sogSeries, MAX_PTS); s2 = pickSeries(full.stwSeries, MAX_PTS);
                  label1 = 'SOG'; label2 = 'STW'; unit = 'kn'; break;
                case 'depth':
                  s1 = pickSeries(full.depthSeries, MAX_PTS);
                  label1 = 'Depth'; unit = 'm'; break;
              }
              return { chartType, s1, s2, label1, label2, unit,
                startTime: full.startTime, endTime: full.endTime };
            }

            // Use getCachedStats — series are already stored in the stats cache
            // (both memory and disk .stats.json). This avoids a full re-parse.
            // Falls back to fresh parse only if cache is cold (first request ever).
            try {
              const cached = getCachedStats(fp);
              if (cached && (cached.sogSeries || cached.twsSeries || cached.depthSeries)) {
                setStatsCacheHeader(res, baseFn);
                res.end(JSON.stringify(buildResponse(cached, chartType)));
                return;
              }
            } catch (e) { /* fall through to async parse */ }

            // Cache cold or series missing — parse async so event loop stays free
            parseLogFileAsync(fp, config, t, {}).then(full => {
              setSeriesCache(fp, full);
              setStatsCacheHeader(res, baseFn);
              res.end(JSON.stringify(buildResponse(full, chartType)));
            }).catch(e => { res.writeHead(500); res.end(JSON.stringify({ error: e.message })); });
            return;
          }

          if (action === 'clean') {
            // Export polar-ready NMEA log: strips AIS, engine-on periods, optionally non-essential sentences.
            // ?strict=1  → keep only polar-essential sentences (RMC, MWV, VHW, VWR, HDG, HDT, VTG, GGA, XDR, RPM)
            // ?raw=1     → also strip ISO timestamp prefixes (for direct replay tools)
            const strict = url.searchParams.get('strict') === '1';
            const rawTs  = url.searchParams.get('raw')    === '1';

            // Read log content (handles .gz transparently)
            const content = readLogContent(fp);
            const ef = readEventsFile(baseFn);

            // ── Build engine-on periods from manual engine events ──────────
            // Primary: use engineState field (new format)
            // Fallback: use detail/note text matching (legacy format)
            const allEv = (ef.manualEvents || []).slice().sort(
              (a, b) => (a.time || '').localeCompare(b.time || ''));
            const enginePeriods = [];
            const motorRe  = /motor|engine/i;
            const startRe  = /start|aan|on|gestart/i;
            const stopRe   = /stop|uit|off|gestopt/i;
            let engineOnAt = null;
            for (const ev of allEv) {
              if (ev.type === 'engine' && ev.engineState) {
                // New format: explicit engineState field
                if (ev.engineState === 'on')  { engineOnAt = new Date(ev.time); }
                if (ev.engineState === 'off' && engineOnAt) {
                  enginePeriods.push({ from: engineOnAt, to: new Date(ev.time) });
                  engineOnAt = null;
                }
              } else {
                // Legacy: text matching on detail/note
                const isEng = motorRe.test(ev.detail || '') || motorRe.test(ev.note || '');
                if (!isEng) continue;
                const txt = (ev.detail || '') + ' ' + (ev.note || '');
                if (startRe.test(txt))               { engineOnAt = new Date(ev.time); }
                else if (stopRe.test(txt) && engineOnAt) {
                  enginePeriods.push({ from: engineOnAt, to: new Date(ev.time) });
                  engineOnAt = null;
                }
              }
            }
            // Engine still on at end of log → exclude to end of file
            if (engineOnAt) enginePeriods.push({ from: engineOnAt, to: new Date('2099-12-31') });

            // ── Essential sentence list for strict mode ────────────────────
            const POLAR_SENTENCES = ['RMC','MWV','VHW','VWR','HDG','HDT','HDM','VTG','GGA','GLL','GNS','XDR','RPM'];
            const isPolar = l => POLAR_SENTENCES.some(s => l.indexOf(s) >= 0);
            ...
            for (let line of content.split('\n')) {
              const l = line.trim();
              if (!l) continue;
              // Strip timestamp prefix to get raw NMEA sentence for analysis
              const nmea = l.replace(TS_RE, '');
              // Skip AIS sentences
              if (nmea.charAt(0) === '!' || nmea.indexOf('AIVDM') >= 0 || nmea.indexOf('AIVDO') >= 0) continue;
              // Track current time via RMC for engine period matching
              if (nmea.indexOf('RMC') >= 0) {
                const p = nmea.split(',');
                if (p.length >= 10 && p[1] && p[9] && p[1].length >= 6 && p[9].length === 6) {
                  curTime = new Date(Date.UTC(
                    parseInt(p[9].substring(4,6)) + 2000,
                    parseInt(p[9].substring(2,4)) - 1,
                    parseInt(p[9].substring(0,2)),
                    parseInt(p[1].substring(0,2)),
                    parseInt(p[1].substring(2,4)),
                    Math.floor(parseFloat(p[1].substring(4)))
                  ));
                }
              }
              // Skip engine-on periods
              if (curTime && enginePeriods.length) {
                let inEng = false;
                for (const ep of enginePeriods) {
                  if (curTime >= ep.from && curTime <= ep.to) { inEng = true; break; }
                }
                if (inEng) continue;
              }
              // Strict mode: only polar-essential sentences
              if (strict && !isPolar(nmea)) continue;
              // Output: raw NMEA or with timestamp prefix
              out.push(rawTs ? nmea : l);
            }

            const cleanContent = out.join('\n') + '\n';
            const suffix = strict ? '_polar_strict.log' : '_polar.log';
            const dlName = baseFn.replace('.log', suffix);

            // ?save=1 → save to exports/ subdirectory (read by polar plugin, separate from raw logs)
            if (url.searchParams.get('save') === '1') {
              const exportsDir = path.join(logDir, 'exports');
              if (!fs.existsSync(exportsDir)) fs.mkdirSync(exportsDir, { recursive: true });
              const savePath = path.join(exportsDir, dlName);
              try {
                fs.writeFileSync(savePath, cleanContent);
                const sizeKB = Math.round(Buffer.byteLength(cleanContent) / 1024);
                res.end(JSON.stringify({
                  ok: true, file: dlName, path: savePath,
                  kept: out.length, sizeKB
                }));
              } catch (e) {
                res.writeHead(500);
            ...
            res.writeHead(200, {
              ...
              'Content-Disposition': `attachment; filename="${dlName}"`,
              'Content-Length': Buffer.byteLength(cleanContent)
            });
            res.end(cleanContent); return;
          }

          if (action === 'download') {
            const raw = url.searchParams.get('raw') === '1';
            // Helper: strip ISO timestamp prefix from each line for VDR compatibility.
            // Pattern: "2026-05-30T08:00:00.000Z $GPRMC,..." → "$GPRMC,..."
            ...
            if (resolved.gz) {
              let content = readLogContent(fp);
              if (raw) content = stripTimestamps(content);
              const filename = raw ? baseFn.replace('.log', '.vdr.log') : baseFn;
              res.writeHead(200, { 'Content-Type':'application/octet-stream', 'Content-Disposition':`attachment; filename="${filename}"`, 'Content-Length': Buffer.byteLength(content) });
              res.end(content); return;
            }
            if (raw) {
              const content = stripTimestamps(fs.readFileSync(fp, 'utf8'));
              const filename = baseFn.replace('.log', '.vdr.log');
              res.writeHead(200, { 'Content-Type':'application/octet-stream', 'Content-Disposition':`attachment; filename="${filename}"`, 'Content-Length': Buffer.byteLength(content) });
              res.end(content); return;
            }
            const stat = fs.statSync(fp);
  ...
  /**
   * Background-warm the stats cache for recent logs.
   * Uses parseLogFileAsync so the event loop stays free.
   * Processes files sequentially with a 500ms pause between each
   * to avoid SD card thrashing at startup.
   */
  function prewarmStatsCache() {
    ...
    const files = fs.readdirSync(logDir)
      .filter(f => f.startsWith('nmea0183_') && (f.endsWith('.log') || f.endsWith('.log.gz')))
      .sort().reverse()
      .slice(0, 14); // Most recent 14 logs (~2 weeks)
    let idx = 0;
    function next() {
      if (idx >= files.length) return;
      const fn = files[idx++];
      const fp = path.join(logDir, fn);
      const isLive = currentLogDate && fn.includes(currentLogDate);
      if (isLive) { setTimeout(next, 100); return; } // skip live log
      const cachePath = statsCachePath(fp);
      // Already cached and newer than log → skip
      try {
        if (fs.existsSync(cachePath)) {
          const cm = fs.statSync(cachePath).mtimeMs;
          const lm = fs.statSync(fp).mtimeMs;
          if (cm >= lm) { setImmediate(next); return; }
        }
      } catch (e) {}
      // Build async, then wait 500ms before next to avoid SD card saturation
      parseLogFileAsync(fp, config, t, {}).then(data => {
        try {
          const diskData = Object.assign({}, data);
          delete diskData.aisVessels;
          fs.writeFileSync(cachePath, JSON.stringify(diskData));
          const memKey = `${fp}:${fs.statSync(fp).mtimeMs}`;
          statsCache[memKey] = { data, time: Date.now() };
          app.debug(`Pre-warmed: ${fn}`);
        } catch (e) { app.debug(`Pre-warm write error ${fn}: ${e.message}`); }
        setTimeout(next, 500); // pace SD card writes
      }).catch(e => {
        app.debug(`Pre-warm failed ${fn}: ${e.message}`);
        setTimeout(next, 500);
      });
    }
    // Start after 10s so SignalK fully initialises first
  ...
  plugin.stop = function () {
    ...
    if (currentWriteStream) {
      // Build stats cache for the current log before closing
      if (currentLogDate) {
        const suffix = currentFilePart > 0 ? `_part${currentFilePart}` : '';
        buildStatsCacheInBackground(`nmea0183_${currentLogDate}${suffix}.log`);
      }
      currentWriteStream.end(); currentWriteStream = null; currentLogDate = null;
    }
  ...
  plugin.schema = function () {
    ...
    return {
      ...
      properties: {
        ...
        includeTimestamp: { type:'boolean', title:'Include ISO Timestamp prefix on each line', description:'Recommended: ON. Prepends each logged sentence with an ISO timestamp (e.g. "2026-05-30T12:34:56.789Z $GPRMC,..."). Required for accurate analysis when GPS sentences are absent. Disable only if you need pure NMEA0183 output and accept loss of timing info.', default:true },
        ...
        fileManagement: { type:'object', title:'File Management', properties: {
          maxFileSizeMB: { type:'number', title:'Max File Size (MB)', description:'New part file when exceeded. 0 = unlimited. Default: 250', default:250 },
          movingSogThresholdKn: { type:'number', title:'Moving Speed Threshold (kn)', description:'SOG below this value is considered "not moving" and excluded from average speed (e.g. at anchor, in marina). Default: 0.5', default:0.5 },
          vdoFallbackSec: { type:'number', title:'AIVDO fallback threshold (s)', description:'Use own-vessel AIS (VDO) position as track fallback when no RMC received for this many seconds. Requires includeTimestamp ON. Class B AIS transmits every 30s. Default: 60. Set 0 to disable.', default:60 },
